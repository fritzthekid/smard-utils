...
from smard_utils.battery_simulation import (
    BatteryManagementSystem,
    BatterySimulation,
    battery_simulation_version,
)
...
class Analyse(BatterySimulation):
    ...
    def __init__(self, data=None, basic_data_set={}, logger=logger, **kwargs):
        ...
        if "battery_model" in kwargs and battery_simulation_version > "0.9":
            super().__init__(
                data=data,
                basic_data_set=basic_data_set,
                battery_model=kwargs["battery_model"],
            )
        else:
    ...
    def prepare_price(self):
        if self.year is None:
            ...
            hours_diff = ((self.data.index - start_time).total_seconds() / 3600).astype(
                int
            )
            ...
            self.data["avrgprice"] = (
                costs["avrgprice"].iloc[hours_diff].values
                + self.basic_data_set["marketing_costs"]
            )
    ...
    def prepare_data(self):
        if "battery_discharge" in self.basic_data_set:
            self.battery_discharge = (
                max(0, min(1, self.basic_data_set["battery_discharge"]))
                * self.resolution
            )
        else:
        ...
        self.exflow = exflow
        share = (
            sum(self.pos) / sum(self.data["my_demand"])
            if sum(self.data["my_demand"]) > 0
            else 0
        )
        ...
        savings_no = "NN"
        savings = "0.00 \N{EURO SIGN}/MWh"
        if self.battery_results_pattern is not None:
            no_ren = [-1, 0, 0, 0, 0, 0, 0]
            no_bat = [
                -1,
                0.0,
                (self.data["my_renew"].sum()),
                0.0,
                0.0,
                0.0,
                (self.data["my_renew"] * self.data["price_per_kwh"]).sum(),
            ]
        else:
            no_ren = [
                -1,
                self.data["my_demand"].sum(),
                0,
                0,
                spot_price_no,
                fix_price_no,
    ...
    def print_results(self):
        print(
            f"reference region: {self.region}, demand: {(sum(self.data['total_demand'])/1000):.2f} GWh, solar: {(sum(self.data['solar'])/1000):.2f} GWh, wind {(sum(self.data['wind_onshore'])/1000):.2f} GWh"
        )
        print(
            f"total demand: {(sum(self.data['my_demand'])/1e3):.2f} MWh "
            + f"total Renewable_Source: {(sum(self.data['my_renew'])/1e3):.2f} MWh"
        )
        print(
            f"total renewalbes: {(sum(self.pos)/1000):.2f} MWh, residual: {(sum(self.neg)/1000):.2f} MWh"
        )
        if self.my_total_demand == 0.0:
            print("share without battery 0.0")
        else:
    ...
    def visualise(self, start=0, end=None):
        ...
        ax1.plot(
            self.data.index[start:end], self.data["my_renew"][start:end], color="green"
        )
        ax1.plot(
            self.data.index[start:end], self.data["my_demand"][start:end], color="red"
        )
        ax1.set_ylabel("[kWh]")
        ...
        ax2.plot(
            ...
            np.maximum(0, self.data["my_renew"][start:end] - self.data["my_demand"])[
                start:end
            ],
            color="green",
        )
        ax2.plot(
            self.data.index[start:end],
            np.minimum(
                0, self.data["my_renew"][start:end] - self.data["my_demand"][start:end]
            ),
            color="red",
        )
        ...
        ax3.plot(
            self.data.index[start:end],
            self.data["battery_storage"][start:end],
            color="blue",
        )
        ...
        ax3.set_ylabel("[kWh]")
        ax4.plot(
            self.data.index[start:end], self.data["residual"][start:end], color="red"
        )
        # ax4.legend(["-Demand"])
...
def main(argv=[]):
    ...
    analyzer.run_analysis(
        capacity_list=[0.1, 1.0, 5, 10, 20],  # , 100],
