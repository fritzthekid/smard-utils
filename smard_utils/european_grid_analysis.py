...
class EuropeanGridAnalyzer:
    ...
    def create_enhanced_renewable_scenario(self):
        ...
        print(
            f"- Expanding (factor: {self.expansion_wind}) German wind onshore and offshore"
        )
        print(f"- Expanding (factor: {self.expansion_solar}) German photovoltaic")
        print("- Keeping other German sources unchanged")
        ...
        for renewable, expansion in zip(renewables_to_double, expansion_list):
            if renewable in scenario.columns:
                ...
                print(
                    f"  {renewable}: {original/1000:.0f} → {enhanced/1000:.0f} GWh (+{(enhanced-original)/1000:.0f} GWh)"
                )
    ...
    def simulate_european_imports(self, month_factor=True):
        ...
        for source_name, source_config in self.interconnections.items():
            ...
            print(
                f"{source_config['description']:.<35} {capacity_mw/1000:4.0f} GW capacity, {generation.mean()/1000:4.2f} GW average"
            )
    ...
    def analyze_with_european_grid(
        self, german_battery_gwh=0, norwegian_reservoir_twh=20
    ):
        """Analyze energy balance including European grid integration"""
        df = self.simulate_european_imports()

        print("\n" + "=" * 60)
        print("ANALYSIS WITH EUROPEAN GRID INTEGRATION")
        print("=" * 60)
        print(f"German battery: {german_battery_gwh} GWh")
        print(
            f"Norwegian reservoir capacity: {norwegian_reservoir_twh} TWh (for reference)"
        )

        # Total renewable supply (German + European)
        df["total_renewable_supply"] = (
            df["german_renewables_total"] + df["european_renewable_imports"]
        )
            ...
            for i in range(len(df)):
                ...
                if balance > 0:  # Surplus
                    max_charge = min(
                        balance,
                        battery_power_mw,
                        battery_capacity_mwh - battery_level[i],
                    )
                    battery_charge[i] = max_charge
                    battery_level[i] += max_charge
                elif balance < 0:  # Deficit
                    max_discharge = min(
                        abs(balance), battery_power_mw, battery_level[i]
                    )
        ...
        results = {
            ...
            "total_renewable_twh": (german_renewable_total + european_import_total)
            / 1e6,
            ...
            "total_renewable_share": (
                german_renewable_total
                + european_import_total
                - curtailment_total
                + (df["battery_charge"].sum() if german_battery_gwh > 0 else 0)
            )
            / total_demand
            * 100,
    ...
    def compare_scenarios(self):
        ...
        for scenario_name, battery_gwh, include_european in scenarios:
            ...
            if include_european:
                ...
                df = self.enhanced_data.copy()
                df["renewable_balance"] = (
                    df["german_renewables_total"] - df["total_demand"]
                )
                ...
                scenario_results = {
                    "total_renewable_share": df["german_renewables_total"].sum()
                    / total_demand
                    * 100,
                    "residual_share": residual_needed / total_demand * 100,
                    "german_renewable_share": df["german_renewables_total"].sum()
                    / total_demand
                    * 100,
            ...
            print(
                f"   German renewables: {scenario_results['german_renewable_share']:5.2f}%"
            )
            if include_european:
                print(
                    f"   European imports:  {scenario_results['european_import_share']:5.2f}%"
                )
            print(
                f"   Total renewable:   {scenario_results['total_renewable_share']:5.2f}%"
            )
    ...
    def create_visualization(self, results):
        ...
        plt.tight_layout()
        plt.savefig(
            f"{root_dir}/doc/fig_european_grid_analysis.png",
            dpi=300,
            bbox_inches="tight",
        )
    ...
    def run_analysis(self):
        ...
        print("\n💡 KEY INSIGHTS:")
        print(
            "   • European grid integration dramatically reduces residual power needs"
        )
        print("   • Cross-border renewable trade more effective than domestic storage")
        print("   • Norwegian hydro acts as massive 'virtual battery' for Europe")
        print("   • Danish offshore wind provides winter renewable supply")
        print("   • Grid integration + modest storage could achieve >95% renewable")
