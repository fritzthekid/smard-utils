...
def get_smard_data(start_date, end_date, module_ids, output_dir="smard_data"):
    ...
    while current_date <= end_date:
        ...
        to_timestamp = int(chunk_end_date.timestamp() * 1000) + (
            24 * 60 * 60 * 1000 - 1
        )  # end of day
        ...
        logging.info(
            f"Requesting data from {current_date.strftime('%Y-%m-%d')} to {chunk_end_date.strftime('%Y-%m-%d')}..."
        )
        ...
        try:
            ...
            with zipfile.ZipFile(io.BytesIO(response.content)) as z:
                for file_info in z.infolist():
                    if file_info.filename.endswith(".csv"):
                        ...
                        os.rename(
                            os.path.join(output_dir, file_info.filename), output_path
                        )
            ...
            logging.error(
                "Downloaded file is not a valid zip file. Response text: %s",
                response.text[:200],
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download electricity generation data from smard.de."
    )
    parser.add_argument(
        "--start",
        type=str,
    ...
    parser.add_argument(
        "--end", type=str, default="2024-12-31", help="End date in YYYY-MM-DD format."
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    logging.info(
        f"Starting download of smard.de data from {args.start} to {args.end}..."
    )
    get_smard_data(
        start_date, end_date, STROMERZEUGUNG_MODULE_IDS, output_dir=args.output
    )
