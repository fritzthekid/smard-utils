...
class BatteryModel:
    def __init__(
        ...
        **kwargs,
        ...
        self.p_max_kw = float(
            p_max_kw or (self.basic_data_set["max_c_rate"] * self.capacity_kwh)
        )
    ...
    def loading_strategie(
        ...
        **kwargs,
        ...
        if strategy == Balance.LOAD:
            ...
            allowed_energy = min(
                power_per_step * dt_h, (self.max_soc * capacity) - current_storage
            )
            actual_charge = min(
                energy_balance, allowed_energy
            )  # kWh aus Überschuss, bevor Verluste
            if actual_charge > 0:
                loss = self._r0_losses(actual_charge / dt_h, dt_h)
                stored_energy = (
                    max(0.0, (actual_charge - loss)) * self.efficiency_charge
                )
        ...
        # elif self.battery_cond_discharge(energy_balance):
            ...
            allowed_energy = f(discharing_factor, df, df_min, sub) * min(
                power_per_step * dt_h, current_storage - self.min_soc * capacity
            )
            ...
            candidate = min(
                allowed_energy, needed / max(self.efficiency_discharge, 1e-9)
            )
            if candidate > 0:
        ...
        current_storage = max(
            self.min_soc * capacity, min(self.max_soc * capacity, current_storage)
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            p_max_kw=p_max_kw,
            init_storage_kwh=init_storage_kwh,
            i=i,
        )
        defaults = {
            "load_threshold": 0.9,
...
class BatterySolBatModel(BatteryModel):
    ...
    def loading_strategie(
        ...
        **kwargs,
        ...
        #  best for capacity <= 20 MWh, ok vor >> 20 MWh
        # _, df_min, sub = 1.3, 0.8, 1.0
        if self.battery_cond_export_a(
            energy_balance,
            discharing_factor=discharing_factor,
            df_min=df_min,
            current_storage=current_storage,
            min_soc=self.min_soc,
            limit_soc_threshold=self.limit_soc_threshold,
            capacity=capacity,
        ):
        ...
        # elif discharing_factor < 0 and current_storage <= (self.max_soc - self.limit_soc_threshold) * capacity and current_storage >= self.limit_soc_threshold:
            ...
            if renew > actual_charge and price > 0.0 and self.control_exflow > 0:
                ...
                self._exporting[i] = True
        elif self.battery_cond_export_b(
            energy_balance, price=price, control_exflow=self.control_exflow
        ):
            exflow = max(0, renew)
            if exflow > 0:
                self.exporting[i] = True
...
class BatteryRawBatModel:
    ...
    def balancing(
        self, current_storage, capacity, requested_charge, power_per_step, **kwargs
    ):
        dt_h = kwargs.get("dt_h", 1.0)
        i = kwargs.get("i", 0)
        if i > 175:
        ...
        if requested_charge < 0:  # discharge
            ...
            allowed_energy = f(
                requested_charge,
                min(power_per_step * dt_h, current_storage - self.min_soc * capacity),
            )
            actual_discharge = allowed_energy  # min(renew, allowed_energy)
            if actual_discharge > 0:
                loss = self._r0_losses(actual_discharge / dt_h, dt_h)
