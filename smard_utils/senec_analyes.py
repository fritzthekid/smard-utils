...
from smard_utils.smard_analyse import Analyse, root_dir
...
class Senec(Analyse):
    ...
    def __init__(self, file_path, basic_data_set={}):
        ...
        self.battery_results = pd.DataFrame(
            [[0, 0, 0, 0]],
            columns=["capacity MWh", "residual MWh", "exflow MWh", "share"],
        )
        self.region = "home"
    ...
    def act_simulate_battery(self, capacity=5, factor=1.0, min_cur=-100, max_cur=100):
        level = [0]
        current = np.minimum(
            max_cur, np.maximum(min_cur, self.data["act_battery_current"])
        )
        for i in range(1, self.data.shape[0]):
            ...
            newlevel = max(
                ...
                min(
                    capacity,
                    level[-1]
                    + current.iloc[i] * self.resolution * self.resolution * factor,
                ),
            )
    ...
    def details(self, start=0, end=None):
        ...
        ax1.plot(
            ...
            self.data["battery_inflow"][start:end],
            color="green",
        )
        ax1.plot(
            self.data.index[start:end],
            self.data["act_battery_inflow"][start:end],
            color="red",
        )
        ax1.plot(
            self.data.index[start:end],
            self.data["battery_exflow"][start:end],
            color="blue",
        )
        ax1.plot(
            self.data.index[start:end],
            -self.data["act_battery_exflow"][start:end],
            color="brown",
        )
        ax1.set_ylabel("[kWh]")
        ...
        ax1.grid(True)
        ax2.plot(
            self.data.index[start:end], self.data["battery"][start:end], color="green"
        )
        ax2.plot(
            self.data.index[start:end], self.data["act_battery"][start:end], color="red"
        )
