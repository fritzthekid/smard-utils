...
def remove_holes_from_data(data):
    ...
    """
    avrgdiff = (data["DateTime_x"].iloc[-1] - data["DateTime_x"].iloc[0]) / (
        len(data) - 1
    )
    data["DateTime"] = [
        data["DateTime_x"].iloc[0] + i * avrgdiff
        for i, e in enumerate(data["DateTime_x"])
    ]
    return data
...
class BiogasDriver(EnergyDriver):
    ...
    def load_data(self, csv_file_path: str) -> pd.DataFrame:
        ...
        df["my_renew"] = (
            self.basic_data_set.get("constant_biogas_kw", 0) * self.resolution
        )
