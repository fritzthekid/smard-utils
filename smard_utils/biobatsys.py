...
class BioBatSys(BaseAnalysisSys):
    ...
    def run_analysis(
        self, capacity_list=[1.0, 5, 10, 20, 100], power_list=[0.5, 2.5, 5, 10, 50]
    ):
        """
    ...
    def print_battery_results(self):
        ...
        """
        flex_add_full = self.basic_data_set.get(
        ...
        fcr_per_sim = (
            self.fcr_revenues_l
            if self.fcr_revenues_l
            else [0.0] * len(self.exporting_l)
        )
        ...
        if len(self.exporting_l) > 1:
            print(
                f"exporting {export_hours[1]:.0f} hours but not {self.exporting_l[1][0] * self.resolution:.0f} hours"
        ...
        exflowl = [f"{(self.battery_results['exflow kWh'].iloc[0] / scaler):.1f}"] + [
        ...
        revenue_l = [f"{(no_rule_rev / scaler):.1f}"] + [
            ...
            for rev, flex in zip(
                self.battery_results["revenue [€]"][2:], flex_per_sim[1:]
            )
        ]
