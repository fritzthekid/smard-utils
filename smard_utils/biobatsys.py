...
class BioBatSys(Analyse):
    ...
    def loading_strategie(self, renew, demand, current_storage, capacity, 
        ...
        if price < avrgprice:
            ...
            self.exporting[i] = False
        # revenue_l = [f"{(self.battery_results['revenue [euro_sign]'].iloc[1]/scaler):.1f}"]+[f"{((f+flex_add)/scaler):.1f}" for f in self.battery_results["revenue [euro_sign]"][2:]]
    ...
    def print_battery_results(self):
        ...
        sp0 = self.battery_results["spot price [euro_sign]"].iloc[1]
        fp0 = self.battery_results["fix price [euro_sign]"].iloc[1]
        # rev0 = self.battery_results["revenue [euro_sign]"].iloc[1]
        rev1 = self.battery_results["revenue [euro_sign]"].iloc[1]
        ...
        if abs(self.data["my_renew"].sum())/1000 > 1000:
            scaler=1000
            cols = ["cap MWh","exfl MWh", "rev [Teuro_sign]", "revadd [Teuro_sign]", "rev euro_sign/kWh"]
        else:
            scaler=1
            cols = ["cap kWh","exfl kWh", "rev [euro_sign]", "revadd [euro_sign]", "rev euro_sign/kWh"]
        ...
        # spot_price_l = [f"{(s/scaler):.1f}" for s in self.battery_results["spot price [euro_sign]"][1:]]
        revenue_l = [f"{(self.battery_results['revenue [euro_sign]'][1]/scaler):.1f}"]+[f"{((f+flex_add)/scaler):.1f}" for f in self.battery_results["revenue [euro_sign]"][2:]]
        revenue_gain = [f"nn"] + [f"{((r-rev1+flex_add)/scaler):.2f}" for r in self.battery_results["revenue [euro_sign]"][2:]]
        capacity_costs = [f"{0:.2f}",f"{0:.2f}"] + [f"{((r-rev1+flex_add)/max(1e-10,c)):.2f}" for r,c in zip(self.battery_results["revenue [euro_sign]"][3:],self.battery_results["capacity kWh"][3:])]
