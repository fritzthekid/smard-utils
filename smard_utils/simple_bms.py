...
class BatteryManagementSystem:
    ...
    def __init__(
        self, data=None, basic_data_set=None, battery_model=BatteryRawBatModel, **kwargs
    ):
    ...
    def battery_cond_load(
        ...
        return (
            discharing_factor < 0
            and current_storage <= (max_soc - limit_soc_threshold) * capacity
            and current_storage >= limit_soc_threshold
        )

    def battery_cond_export_a(
        self,
        energy_balance,
        discharing_factor,
        df_min,
        ...
        return (
            discharing_factor > df_min
            and current_storage >= (min_soc + limit_soc_threshold) * capacity
            and current_storage >= -limit_soc_threshold
        )
    ...
    def run_management(
        ...
        **kwargs,
        ...
        [current_storage, inflow, outflow, batexflow, loss] = self.battery.balancing(
            current_storage, capacity, requested_charge, power_per_step
        )
...
class BatterySimuation:
    ...
    def __init__(
        self,
        battery_model=BatteryRawBatModel,
        ...
        **kwargs,
        ...
        self.data = data
        res = (
            (self.data.index[-1] - self.data.index[0]) / self.data.index.shape[0]
        ) / pd.Timedelta("1 hour")
        ...
        self.battery_management_system = battery_management_type(
            data=self.data,
            basic_data_set=self.basic_data_set,
    ...
    def prepare_price(self, costs):
        ...
        # Nutze iloc mit Array-Indexing (viel schneller!)
        self.data["price_per_kwh"] = (
            costs["price"].iloc[hours_diff].values
            + self.basic_data_set["marketing_costs"]
        )
    ...
    def setup_costs(self, raw_costs, timeframe=None):
        ...
        if self.basic_data_set.get("fix_contract", False):
            ...
            timeing = pd.date_range(
                start=timeframe[0] - pd.Timedelta(hours=24),
                end=timeframe[1] + pd.Timedelta(hours=24),
                freq=self.time_resolution,
            )
            costs = pd.DataFrame(
                fix_costs_per_kwh + 0 * timeing, index=timeing, columns=["costs"]
            )
            self.average_costs = fix_costs_per_kwh
        else:
            if type(raw_costs) != pd.DataFrame:
                raise ValueError(
                    "for variable costs contract, raw_costs must be a DataFrame"
                )
            self.average_costs = raw_costs["price"].mean()
            self.costs_resolution = np.round(
                (
                    (raw_costs.index[-1] - raw_costs.index[0])
                    / raw_costs.index.shape[0]
                ).seconds
                / 60
            )
            ...
            if raw_costs.index[0] > self.data.index[0]:
                start_index = pd.date_range(
                    start=raw_costs.index[0] - pd.Timedelta(hours=24),
                    end=self.data.index[0] - self.cost_resolution,
                    freq=self.costs_resolution,
                )
                start_frame = pd.DataFrame(
                    [self.average_costs for i in start_index],
                    columns=["costs"],
                    index=start_index,
                )
            else:
                start_index = pd.date_range(
                    start=raw_costs.index[0] - pd.Timedelta(hours=24),
                    end=raw_costs.index[0]
                    - pd.Timedelta(seconds=int(self.costs_resolution)),
            ...
            if raw_costs.index[-1] < self.data.index[-1]:
                end_index = pd.date_range(
                    start=self.data.index[-1]
                    + pd.Timedelta(seconds=int(60 * self.costs_resolution)),
                    end=raw_costs.index[-1] + pd.Timedelta(hours=24),
                    freq=pd.Timedelta(seconds=int(60 * self.costs_resolution)),
                )
                end_frame = pd.DataFrame(
                    [self.average_costs for i in end_index],
                    columns=["costs"],
                    index=end_index,
                )
            else:
                end_index = pd.date_range(
                    start=raw_costs.index[-1] + self.cost_resolution,
                    end=raw_costs.index[-1] + pd.Timedelta(hours=24),
                    freq=self.costs_resolution,
                )
                end_frame = pd.DataFrame(
    ...
    def do_simulation(self, **kwargs):
        capacity = kwargs.get(
            "capacity", self.basic_data_set.get("capacity_kwh", 2000.0)
        )
        ...
        self.battery_management_system.init_inport_export_modelling(
            exporting=np.zeros(len(self.data["my_demand"]))
        )
        ...
        for i, (r, d) in enumerate(zip(self.data["my_renew"], self.data["my_demand"])):
            ...
            stor, inf, outf, resi, exf, los = (
                self.battery_management_system.run_management(
                    renew=r,
                    demand=d,
        ...
        self.exporting_l.append(
            (
                np.size(self.battery_management_system.exporting())
...
class Analyse:
    ...
    def __init__(
        ...
        data,
        basic_data_set={},
        battery_simulation=BatterySimuation,
        logger=None,
        **kwargs,
    ):
        ...
        self.battery_simulation = battery_simulation(
            basic_data_set=self.basic_data_set, data=self.data, logger=logger
        )
    ...
    def load_and_prepare_data(self, csv_file_path, **kwargs):
        ...
        df["my_demand"] = (
            df["total_demand"] * my_total_demand / total_demand * self.resolution
        )
        df["my_renew"] = (
            df["wind_onshore"]
            * self.basic_data_set["wind_nominal_power"]
            / total_installed_wind
            * self.resolution
        )
        df["my_renew"] += (
            df["solar"]
            * self.basic_data_set["solar_max_power"]
            / total_installed_solar
            * self.resolution
        )
    ...
    def run_analysis(
        self, capacity_list=[5000, 10000, 20000], power_list=[2500, 5000, 10000]
    ):
        """Run the analysis"""
        ...
        if DEBUG:
            results = self.battery_simulation.do_simulation(
                capacity=5 * 1000, power=2.5 * 1000
            )
        battery_results = None
        for capacity, power in zip(capacity_list, power_list):
            results = self.battery_simulation.do_simulation(
                capacity=capacity * 1000, power=power * 1000
            )
    ...
    def print_battery_results(self):
        ...
        spotprice_gain = [f"{0:.2f}", f"{0:.2f}", f"{0:.2f}"] + [
            f"{((sp0-s)/max(1e-10,c)):.2f}"
            for s, c in zip(
                self.battery_results["spot price [\N{EURO SIGN}]"][3:],
                self.battery_results["capacity kWh"][3:],
            )
        ]
        fixprice_gain = [f"{0:.2f}", f"{0:.2f}", f"{0:.2f}"] + [
            f"{((fp0-f)/max(1e-10,c)):.2f}"
            for f, c in zip(
                self.battery_results["fix price [\N{EURO SIGN}]"][3:],
                self.battery_results["capacity kWh"][3:],
            )
        ]
        ...
        spot_price_l = [
            f"{(s/scaler):.1f}"
            for s in self.battery_results["spot price [\N{EURO SIGN}]"]
        ]
        fix_price_l = [
            f"{(f/scaler):.1f}"
            for f in self.battery_results["fix price [\N{EURO SIGN}]"]
        ]
        values = np.array(
            [
                capacity_l,
    ...
    def print_results_with_battery(self):
        ...
        if self.my_total_demand / 1000 > 1000:
            ...
            unit = "kWh"
        print(
            f"total renewalbes: {(sum(self.pos)/scaler):.2f} {unit}, residual: {(res/scaler):.2f} {unit}, export: {(sum(self.data["exflow"])/scaler):.2f} {unit}"
        )
        print(
            f"share with battery: {((self.my_total_demand - res)/self.my_total_demand):.2f}"
        )
...
def main(argv={}):
    ...
    analyzer = TestAnalyse(
        data_file, region="_lu", basic_data_set=basic_data_set, logger=logger
    )
    if "pytest_path" in argv:
        analyzer.pytest_path = argv["pytest_path"]
    analyzer.run_analysis(
        capacity_list=[1.0, 5, 10, 20, 50, 70],  # , 100],
