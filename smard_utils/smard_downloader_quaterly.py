...
class SmardAPIDownloader:
    ...
    def get_timeseries_data(self, filter_id: str, timestamp: int) -> Dict:
        ...
        try:
            ...
            print(
                f"✗ Error getting data for filter {filter_id}, timestamp {timestamp}: {e}"
            )
            return {}
    ...
    def filter_timestamps_for_year(
        self, timestamps: List[int], year: int = 2024
    ) -> List[int]:
    ...
    def download_filter_data(self, filter_id: str, year: int = 2024) -> pd.DataFrame:
        ...
        for i, timestamp in enumerate(year_timestamps):
            if i > 0 and i % 10 == 0:
                print(
                    f"  Progress: {i}/{len(year_timestamps)} ({i/len(year_timestamps)*100:.1f}%)"
                )
        ...
        if all_data:
            ...
            print(
                f"✓ Downloaded {len(df)} records for {self.filters.get(filter_id, filter_id)}"
            )
            return df
        else:
    ...
    def download_all_data(
        self, year: int = 2024, output_dir: str = "smard_data"
    ) -> None:
    ...
    def combine_all_data(
        self, dataframes: Dict[str, pd.DataFrame], year: int, output_dir: str
    ) -> None:
        ...
        combined_df = dataframes[first_key][
        ...
        for filter_id, df in dataframes.items():
            ...
            combined_df = combined_df.merge(
                df[["Timestamp", value_column]], on="Timestamp", how="outer"
            )
        ...
        combined_df.to_csv(
            output_file, sep=";", decimal=",", index=False, encoding="utf-8"
        )

        print(f"✓ Combined data saved to: {output_file}")
        print(f"Total rows: {len(combined_df)}")
        print(
            f"Date range: {combined_df['Datum'].min()} to {combined_df['Datum'].max()}"
        )
