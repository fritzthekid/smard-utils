...
def ctx(renew, demand, current_storage, capacity, power_limit=100.0, resolution=1.0):
...
def strat():
    return AutoarkyStrategy(
        {"min_soc": 0.05, "max_soc": 0.95, "efficiency_discharge": 0.96}
    )
...
class TestCalculateDischargeAmount:
    ...
    def test_deficit_limited(self, strat):
        deficit = 1.0
        c = ctx(
            renew=1.0, demand=2.0, current_storage=9.0, capacity=10.0, power_limit=100.0
        )
    ...
    def test_default_efficiency(self, strat_defaults):
        ...
        assert strat_defaults.calculate_discharge_amount(c) == pytest.approx(
            expected, rel=1e-3
        )
...
class TestCalculateChargeAmount:
    ...
    def test_surplus_limited(self, strat):
        surplus = 2.0
        c = ctx(
            renew=3.0, demand=1.0, current_storage=4.0, capacity=10.0, power_limit=100.0
        )
    ...
    def test_room_limited(self, strat):
        ...
        c = ctx(
            renew=5.0, demand=0.5, current_storage=9.0, capacity=10.0, power_limit=100.0
        )
    ...
    def test_zero_when_no_surplus(self, strat):
        c = ctx(
            renew=0.5, demand=2.0, current_storage=4.0, capacity=10.0, power_limit=100.0
        )
    ...
    def test_zero_when_battery_full(self, strat):
        # room = 9.5 - 9.5 = 0
        c = ctx(
            renew=3.0, demand=0.5, current_storage=9.5, capacity=10.0, power_limit=100.0
        )
        assert strat.calculate_charge_amount(c) == pytest.approx(0.0, abs=1e-9)
