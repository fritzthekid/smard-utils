...
class TestBatteryAnalytics:
    ...
    def test_prepare_prices_fixed_contract(self):
        ...
        analytics = BatteryAnalytics(
            driver, {"fix_contract": True, "fix_costs_per_kwh": 10}
        )
    ...
    def test_prepare_prices_missing_file(self):
        ...
        analytics = BatteryAnalytics(
            driver, {"year": 9999, "fix_costs_per_kwh": 11}  # Non-existent year
        )
        analytics.prepare_prices()
    ...
    def test_prepare_prices_with_marketing_costs(self):
        ...
        analytics = BatteryAnalytics(
            driver,
            {"fix_contract": True, "fix_costs_per_kwh": 10, "marketing_costs": 0.02},
        )
        analytics.prepare_prices()

        assert driver.data["price_per_kwh"].iloc[0] == pytest.approx(
            0.12
        )  # 0.10 + 0.02
    ...
    def test_revenue_calculation(self):
        ...
        analytics = BatteryAnalytics(
            driver, {"fix_costs_per_kwh": 11, "marketing_costs": -0.003}
        )
        analytics.prepare_prices()
